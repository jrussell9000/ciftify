    ...
    for hemisphere, structure in [('L', 'CORTEX_LEFT'), ('R', 'CORTEX_RIGHT')]:
        ...
        if os.path.exists(fs_annot):
            ...
            run(['mris_convert', '--annot', fs_annot,
                os.path.join(fs_folder, 'surf', '{}h.white'.format(hemisphere.lower())),
            ...
            run(['wb_command', '-set-map-names', label_gii,
                '-map', '1', '{}_{}_{}'.format(subject_id, hemisphere, label_name)], dryrun=DRYRUN)
...
def apply_nonlinear_warp_to_surface(subject_id, surface, reg_settings, meshes):
    ...
    for hemisphere, structure in [('L', 'CORTEX_LEFT'), ('R', 'CORTEX_RIGHT')]:
        ...
        run(['wb_command', '-surface-apply-warpfield', surf_dest,
            ...
            surf_dest, '-fnirt', os.path.join(xfms_dir, reg_settings['AtlasTransform_NonLinear'])])
...
def convert_freesurfer_maps(subject_id, map_dict, fs_folder,
    ...
    and set metadata'''
    print("Converting a freesurfer data (thickness, curv, sulc) to a gifti metric and set metadata")
    for hemisphere, structure in [('L', 'CORTEX_LEFT'), ('R', 'CORTEX_RIGHT')]:
        map_gii = metric_file(subject_id, map_dict['mapname'], hemisphere,
                              dest_mesh_settings)
        print(f"map_gii: {map_gii}")
        print(f"map dict: {map_dict}")
        ## convert the freesurfer files to gifti
        run(['mris_convert', '-c',
            os.path.join(fs_folder, 'surf', '{}h.{}'.format(hemisphere.lower(), map_dict['fsname'])),
            os.path.join(fs_folder, 'surf', '{}h.white'.format(hemisphere.lower())),
            map_gii], dryrun=DRYRUN)

        map_gii_path = '/'.join(map_gii.split('/')[:-1])
        old_gii_file = '.'.join([f'{hemisphere.lower()}h', map_gii.split('/')[-1]])
        old_gii = '/'.join([map_gii_path, old_gii_file])
        print(old_gii)

        print(f'Bad Gii: {old_gii}')
        if os.path.exists(old_gii):
            os.rename(old_gii, map_gii)
...
def create_reg_sphere(settings, subject_id, meshes):
    ...
    run_fs_reg_LR(subject_id, settings.ciftify_data_dir, settings.high_res, settings.debug,
...
def run_fs_reg_LR(subject_id, ciftify_data_dir, high_res_mesh, debug, reg_sphere,
...
def calc_areal_distortion_gii(sphere_pre, sphere_reg, debug, AD_gii_out, map_prefix,
                              map_postfix):
    ...
    '''
    with ciftify.utils.TempDir(debug) as va_tmpdir:
...
def main():
    ...
    with ciftify.utils.TempDir(debug) as tmpdir:
